...
- **Открыть предопределенные данные** (`metadataViewer.openPredefinedData`) - открывает редактор предопределенных элементов для справочников, документов и других объектов, поддерживающих предопределенные элементы. Позволяет редактировать, добавлять и удалять предопределенные элементы с сохранением структуры XML
...
- **Открыть форму** (`metadataViewer.openForm`) - открывает модуль формы (`Module.bsl`) для редактирования
- **Предпросмотр формы** (`metadataViewer.previewForm`) - открывает визуальный предпросмотр формы с возможностью просмотра элементов, команд, реквизитов и других свойств формы.
...
- **Очистить фильтр по подсистеме** (`metadataViewer.clearFilter`) - снимает активный фильтр по подсистеме. Доступна через контекстное меню подсистемы, по которой применён фильтр.

- **Фильтровать по подсистеме** (`metadataViewer.selectSubsystemToFilter`) - открывает диалог выбора подсистемы для фильтрации. Доступна через контекстное меню корня конфигурации.
