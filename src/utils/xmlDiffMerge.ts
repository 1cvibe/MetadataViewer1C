...
import { deepClone } from './xmlStructureUtils';

export class XmlDiffMerge {
  /**
   * Рекурсивно сравнивает два объекта и возвращает различия
   */
  private static diff(original: any, changed: any, path: string = ''): any {
    if (original === changed) {
   ...
   */
  private static applyDiff(original: any, diff: any): any {
    if (diff === undefined) {
    ...
    if (Array.isArray(diff)) {
      ...
      for (let i = 0; i < maxLength; i++) {
        ...
        if (i >= diff.length) {
          ...
          if (i < original.length) result.push(deepClone(origItem));
          continue;
        }
   ...
   */
  static merge(original: any, changed: any): any {
    if (!original) {
      return changed;
    }
   ...
   */
  static hasChanges(original: any, changed: any): boolean {
    const diff = this.diff(original, changed);
