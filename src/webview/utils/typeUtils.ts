...
export interface TypeValue {
  // Различные форматы типа
  kind?: string;  // ParsedTypeRef
  'v8:Type'?: string | { '#text': string };
 ...
 */
export function extractTypeString(type: any): string | null {
  if (!type) return null;
...
export function formatTypeForDisplay(type: any): string {
  ...
  if (typeof type === 'object' && type !== null) {
    ...
    if (type.Type && Array.isArray(type.Type)) {
      const types = type.Type.map((t: any) => {
        let tStr = '';
    ...
    if (type.kind === 'Composite' && type.details && type.details.Type && Array.isArray(type.details.Type)) {
      const types = type.details.Type.map((t: any) => {
 ...
 */
export function normalizeTypeForSave(typeValue: string, referenceTypesList: Array<{ value: string }>): string {
